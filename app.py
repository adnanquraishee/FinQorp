...
from modules import data_fetch, sentiment, forecast, fundamentals, insights, compare, recommendation, ticker_resolver, technicals, accuracy
...
def set_analysis_ticker(ticker_symbol):
    ...
    with st.spinner(f"Generating forecasts and ratings for {ticker_symbol}..."):
        try:
            ...
            _, sentiment_fig, sentiment_score = sentiment.analyze_sentiment(ticker_symbol)
            
            # 2. Generate forecast (for 30 days for rating, 90 for plot) - USING 200 SIMULATIONS
            hist_df_30d, simulations_30d, _ = forecast.generate_forecast(ticker_symbol, period=30, num_simulations=200)
            ...
            hist_df_90d, simulations_90d, future_dates_90d = forecast.generate_forecast(ticker_symbol, period=90, num_simulations=200)
            forecast_fig = forecast.plot_forecast(hist_df_90d, simulations_90d, future_dates_90d, sentiment_score)
            
            # 5. Run accuracy backtest (30-day)
            accuracy_results = accuracy.run_backtest(ticker_symbol, forecast_days=30, num_simulations=200)
            ...
            st.session_state.forecast_failed = True
...
with main_container:
    ...
    # --- BLOCK 2: Disambiguation (if needed) ---
    ...
    # --- BLOCK 3: Page Renderer ---
    else:
        # Load data from session state if analysis is ready
        if st.session_state.analysis_ready:
        ...
        # --- PAGE 1: Overview (Refined Full-Width Dashboard) ---
        if st.session_state.page == 'overview':
            if st.session_state.analysis_ready:
                ...
                with st.container(border=True):
                    ...
                    # Display ALL metrics in a grid of 4 columns
                    metric_items = list(metrics.items())
                    rows = [metric_items[i:i + 4] for i in range(0, len(metric_items), 4)]
                    for row in rows:
                        cols = st.columns(4)
                        for i, (k, v) in enumerate(row):
                            cols[i].metric(k, v)

                # --- ROW 2: Interactive Chart (Full Width) ---
                with st.container(border=True):
                    ...
                    with st.spinner("Loading chart data..."):
                        ...
                        if not chart_data.empty:
                            ...
                            fig.update_layout(
                                title=dict(text=f"<b>{ticker} Price Action</b>", font=dict(color='#FFFFFF')),
                                height=600, # Taller for full width
                ...
                # --- ROW 3: Recent Data Table (Full Width) ---
                with st.container(border=True):
                    st.subheader("Recent Market Data")
                    with st.spinner("Fetching recent price data..."):
                        hist_5d = data_fetch.get_stock_data(ticker, period="10d", interval="1d")
                ...
                # --- ROW 4: Info Section (Profile Left, Briefing Right) ---
                col_profile, col_brief = st.columns(2)
            ...
            # --- ROW 5: Market & Sentiment Analysis (Larger) ---
            with st.container(border=True):
                st.subheader("Market & Sentiment Analysis")
                
                # Combine historical figs and sentiment fig
                all_figs = list(figs.values()) if figs else []
                if st.session_state.sentiment_fig:
                    all_figs.append(st.session_state.sentiment_fig)
                
                if all_figs:
                    # Use 2 columns for larger charts
                    col_figs = st.columns(2) 
        ...
        # --- PAGE 2: Technical Analysis ---
            ...
            if not st.session_state.analysis_ready:
                st.info("Please search for a stock to view technical analysis.")
            else:
                st.title(f"SYMBOL: {ticker} | Technical Analysis")
                
                if technicals is None:
                    st.error("Technical analysis module (technicals.py) not found.")
                else:
        ...
        # --- PAGE 3: Forecast and Recommendation ---
            ...
            if not st.session_state.analysis_ready:
                st.info("Please search for a stock to view forecasts and ratings.")
            else:
                ...
                # --- NEW SECTION: Forecast Accuracy Testing ---
                st.subheader("3. Forecast Accuracy Testing")
                st.markdown('<div class="panel">', unsafe_allow_html=True)
                
                if st.session_state.accuracy_results and 'error' not in st.session_state.accuracy_results:
                    results = st.session_state.accuracy_results
                    metrics = results.get('metrics', {})
                    acc_score = results.get('accuracy_score', 0)
                    
                    # Display accuracy score with gauge-like display
                    col_score, col_info = st.columns([1, 2])
                    
                    with col_score:
                        # Create a colored metric based on score
                        if acc_score >= 70:
                            score_color = "🟢"
                        ...
                        st.markdown(f"""
                        <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); border-radius: 10px;">
                            <h1 style="color: white; font-size: 3em; margin: 0;">{score_color}</h1>
                            <h2 style="color: white; margin: 10px 0;">{acc_score:.1f}/100</h2>
                            <p style="color: #e0e0e0; margin: 0;">{interpretation}</p>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    with col_info:
                        st.markdown("**About the Accuracy Score:**")
                        st.markdown("""
                        The accuracy score (0-100) evaluates forecast quality through:
                        - **Directional Accuracy** (50%): How often the forecast predicted the correct price direction
                        - **MAPE** (30%): Mean absolute percentage error
                        - **RMSE** (20%): Root mean square error relative to price
                        
                        Scores above 55 indicate better-than-random prediction capability.
                        """)
                    
                    st.markdown("---")
                    
                    # Display metrics table
                    st.markdown("**Detailed Accuracy Metrics** (30-day backtest):")
                    metrics_df = accuracy.format_metrics_table(metrics, acc_score)
                    st.dataframe(metrics_df, use_container_width=True, hide_index=True)
                    
                    st.markdown("---")
                    
                    # Display backtest plot
                    st.markdown("**Backtest Visualization:** Actual vs Predicted Prices")
                    backtest_fig = accuracy.plot_backtest_results(results)
                    st.pyplot(backtest_fig)
                    
                elif st.session_state.accuracy_results and 'error' in st.session_state.accuracy_results:
                    st.warning(f"⚠️ Could not run accuracy test: {st.session_state.accuracy_results['error']}")
                else:
                    st.info("Accuracy testing was not run for this forecast.")
        ...
        # --- PAGE 4: Comparative Analysis ---
            ...
            # --- MODIFICATION: Set a static default to prevent .NS error ---
            comp_input = st.text_input("COMPARISON LIST", 
                                       value="AAPL, MSFT, GOOG, NVDA", 
                                       key="comp_list_input")
